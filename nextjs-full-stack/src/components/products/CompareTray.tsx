                ...
                {compareProducts.map((item) => (
                  <div key={item.id} className="relative group last:mr-4">
                    <Image
                      ...
                      width={64}
                      height={64}
                      className="h-14 w-14 rounded-md object-contain border-2 border-slate-600 hover:border-blue-500"
                    />
                    <button
                      onClick={() => removeFromCompare(item.id)}
                      className="absolute -top-2 -right-2 h-5 w-5 bg-red-600 rounded-full flex items-center justify-center text-white cursor-pointer hover:scale-125 transition-all"
